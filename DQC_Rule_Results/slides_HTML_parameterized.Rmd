...
```{r eval = FALSE}
# need to set working directory if running script not from shiny app
setwd("~/Google_Drive/Projects/DQC_slides/DQC_Rule_Results")
```
...
if (params$data_source_counts=="file"){
  ...
  SELECT r.report_id, e.reference_number AS cik, f.filing_date
  FROM filing f
  JOIN entity e ON f.entity_id = e.entity_id
  JOIN report r ON f.filing_id = r.filing_id
  WHERE (f.filing_date >= ", paste0("\'", params$start_date, "\'"), "AND f.filing_date   < ", paste0("\'", params$end_date, "\'"),") AND f.form_type <> '485BPOS' AND f.form_type <>  '485APOS' AND f.form_type <> '497'
  );
