...
output: 
  ioslides_presentation:
    ...
    smaller: true
params: 
  directory: "~/Google_Drive/Projects/DQ_requests/DQC_slides"
  username: "web_query"
  password: "W0rkiv@"
  ...
  file_input: "DQC_2014_onwards.csv"
  # exclude_rules: c("Rule 0008", "Rule 0011")
  data_source_counts: "file"
  counts_file_input: "gaap_v_ext_unique_dp.csv"
...
```{r eval = FALSE}
  params = list(directory = "/Users/bradwest/Google_Drive/Projects/DQ_requests/DQC_slides",
                                username = "web_query",
                                password = "W0rkiv@",
                                dbserver = "rltest.markv.com",
                                dbport = 8084,
                                dbname = "debug3_db",
                                data_source = "database",
                                file_input = "rulesdata_raw_14_17.csv",
                                data_source_counts = "file",
                                counts_file_input = "gaap_v_ext_unique_dp.csv",
...
```{r make_slides, echo = FALSE, include = F, eval = FALSE}
### TODO:/NOTES:

# The bulk of this code written by Allen Bross and Michael Lerch, 
# modified by b_west to run as a parameterized r-markdown document
...
if (params$data_source=="file"){
  rulesdata <- read_csv(paste0(params$directory, "/", params$file_input))
    ...
    command <- "./query_errors.sh"
    arguments <- c(start_date, end_date, "./errors_results.csv")
    # need to wait until command is finished executing, stderr to error file
    system2(command, args = arguments, wait = TRUE, stderr = "./query_stderr.log")
    rulesdata <- read_csv("./errors_results.csv")
} else {
  library(RPostgreSQL)
...
# all_merge <- merge(merge(merge(merge(merge(merge(merge(merge(merge(merge(merge(merge(merge(merge(ruleByQuarter(rule0001),
#                                                        ruleByQuarter(rule0004), by = "quarter", all = TRUE),
#                                                  ruleByQuarter(rule0005), by = "quarter", all = TRUE),
#                                            ruleByQuarter(rule0006), by = "quarter", all = TRUE),
#                                      ruleByQuarter(rule0008), by = "quarter", all = TRUE),
#                                      ruleByQuarter(rule0009), by = "quarter", all = TRUE),
#                          ruleByQuarter(rule0011), by = "quarter", all = TRUE),
#                    ruleByQuarter(rule0013), by = "quarter", all = TRUE),
# ruleByQuarter(rule0014), by = "quarter", all = TRUE),
#                                ruleByQuarter(rule0015), by = "quarter", all = TRUE),
# ruleByQuarter(rule0018), by = "quarter", all = TRUE),
#                          ruleByQuarter(rule0033), by = "quarter", all = TRUE),
#                    ruleByQuarter(rule0036), by = "quarter", all = TRUE),
# ruleByQuarter(rule0041), by = "quarter", all = TRUE),
#              factsByQuarter(filingsdata), by = "quarter", all = TRUE)
# colnames(all_merge) <- c("quarter","Rule 0001", "Rule 0004", "Rule 0005", "Rule 0006", "Rule 0008",
#                          "Rule 0009", "Rule 0011", "Rule 0013", "Rule 0014", "Rule 0015", "Rule 0018",
#                          "Rule 0033", "Rule 0036", "Rule 0041", "count")
# all_merge <- all_merge[grepl("2014|2015|2016|2017", all$quarter), ]
...
if (params$data_source_counts=="file"){
  ext_count_data <- read_csv(paste0(params$directory, "/", params$counts_file_input))
} else if (params$data_source_counts=="query"){
    # # see _test_query_via_system.R for description
    # command <- paste0("echo '\\i ~/Google_Drive/SQL/current_session.sql \\\\ EXECUTE test_filing('\\''",
    #                   params$start_date, "'\\'', '\\''", params$end_date,
    #                   "'\\'')' | psql -h rltest.markv.com -p8084 -d debug3_db -U postgres -q")
    # # need to wait until command is finished executing, stderr to error file
...
if (!dir.exists(paste0(params$directory, "/plots"))){
  dir.create(paste0(params$directory, "/plots"))
}

# png("./plots/facts_total.png", width = 800, height = 300)
# makeBarplot(all[ , c(1, ncol(all))], singlecol, legend = FALSE, ymax = 1000000000)
# dev.off()
# 
# 
# makeBarplot(all[ , c(1, ncol(all))], singlecol, legend = FALSE, ymax = 500000000)

# Plot styles:
#  source: all, laf, src
#  rules: all, no15, only15
#  scale: count, per1k
#  style: stack, mono
#  year: __, 2014

# # all, all, per1k, mono
# png("./rule_violations_all_all_per1k_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = allp[ , 1], rule = apply(allp[ , 2:(ncol(allp)-1)], 1, sum, na.rm = T) / allp$count * 1000),
#             singlecol, legend = FALSE, ymax = 4)
# dev.off()

## Slide 1
# all, all, per1k, stack
png(paste0(params$directory, "/plots/rule_violations_all_all_per1k_stack.png"), width = 800, height = 300)
...
# # all, all, count, mono
# png("./rule_violations_all_all_count_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = allp[ , 1], rule = apply(allp[ , ruleorder], 1, sum, na.rm = T)),
#             singlecol, legend = FALSE, ymax = 300000)
# dev.off()
# 
# 
# # all, all, count, stack
# png("./rule_violations_all_all_count_stack.png", width = 800, height = 300)
# makeBarplot(allp[ , c(1, ruleorder)], plotcols, legend = TRUE, ymax = 400000)
# dev.off()
# 
# 
# # all, only15, per1k, mono
# png("./rule_violations_all_only15_per1k_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = allp[ , 1], rule = allp[ , which(colnames(allp)=='Rule 0015')] / allp$count * 1000),
#             singlecol, legend = FALSE, ymax = 3)
# dev.off()
# 
# 
# # all, only15, per1k, stack
# png("./rule_violations_all_only15_per1k_stack.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = allp[ , 1], 'rule 0015' = allp[, which(colnames(allp)=='Rule 0015')] / allp$count * 1000),
#             plotcols[2], legend = TRUE, ymax = 3)
# dev.off()
# 
# # all, only01, per1k, mono
# png("./rule_violations_all_only01_per1k_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = allp[ , 1], rule = allp[ , which(colnames(allp)=='Rule 0001')] / allp$count * 1000),
#             singlecol, legend = FALSE, ymax = 3)
# dev.off()
# 
# # all, only01, per1k, stack
# png("./rule_violations_all_only01_per1k_stack.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = allp[ , 1], 'rule 0001' = allp[, which(colnames(allp)=='Rule 0001')] / allp$count * 1000),
#             plotcols[1], legend = TRUE, ymax = 3)
# dev.off()
# 
# # all, no15, no01, per1k, mono
# png("./rule_violations_all_no15_no01_per1k_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = allp[ , 1], rule = apply(allp[ , -c(1, which(colnames(allp)=='Rule 0015'), which(colnames(allp)=='Rule 0001'), ncol(allp))], 1, sum, na.rm = T) / allp$count * 1000),
#             singlecol, legend = FALSE, ymax = 0.5)
# dev.off()

# Slide 4
# all, no15, no01, per1k, stack
png(paste0(params$directory, "/plots/rule_violations_all_no15_no01_per1k_stack.png"), width = 800, height = 300)
makeBarplot(cbind(quarter = allp[ , 1], allp[ , ruleorder[-c(1,2)]] / allp$count * 1000),
            plotcols[-c(1,2)], legend = TRUE, ymax = 2)
dev.off()

# # laf, all, count, mono
# png("./rule_violations_laf_all_count_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = lafp[ , 1], rule = apply(lafp[ , c(-1, -ncol(lafp))], 1, sum)),
#             singlecol, legend = FALSE, ymax = 100000)
# dev.off()
# 
# 
# # laf, all, count, stack
# png("./rule_violations_laf_all_count_stack.png", width = 800, height = 300)
# makeBarplot(lafp[ , c(1, ruleorder)], plotcols, legend = FALSE, ymax = 100000)
# dev.off()

## Slide 2
# laf, all, per1k, stack
png(paste0(params$directory, "/plots/rule_violations_laf_all_per1k_stack.png"), width = 800, height = 300)
makeBarplot(cbind(quarter = lafp[ , 1], lafp[ , ruleorder] / lafp$count * 1000),
            plotcols, legend = TRUE, ymax = 35)
dev.off()

# # src, all, count, mono
# png("./rule_violations_src_all_count_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = srcp[ , 1], rule = apply(srcp[ , -c(1, ncol(srcp))], 1, sum)),
#             singlecol, legend = FALSE, ymax = 30000)
# dev.off()
# 
# 
# # src, all, count, stack
# png("./rule_violations_src_all_count_stack.png", width = 800, height = 300)
# makeBarplot(srcp[ , c(1, ruleorder)], plotcols, legend = FALSE, ymax = 30000)
# dev.off()

# Slide 3
# src, all, per1k, stack
png(paste0(params$directory, "/plots/rule_violations_src_all_per1k_stack.png"), width = 800, height = 300)
makeBarplot(cbind(quarter = srcp[ , 1], srcp[ , ruleorder] / srcp$count * 1000),
            plotcols, legend = TRUE, ymax = 25)
dev.off()


# # laf, no15, no01, count, mono
# png("./rule_violations_laf_no15_no01_count_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = lafp[ , 1], rule = apply(lafp[ , -c(1, which(colnames(lafp)=='Rule 0015'), which(colnames(lafp)=='Rule 0001'), ncol(lafp))], 1, sum, na.rm = T)),
#             singlecol, legend = FALSE, ymax = 5000)
# dev.off()
# 
# 
# # laf, no15, no01, count, stack
# png("./rule_violations_laf_no15_no01_count_stack.png", width = 800, height = 300)
# makeBarplot(lafp[ , c(1, ruleorder[-c(1,2)])],
#             plotcols[-c(1,2)], legend = TRUE, ymax = 5000)
# dev.off()

# Slide 5
# laf, no15, no01, per1k, stack
png(paste0(params$directory, "/plots/rule_violations_laf_no15_no01_per1k_stack.png"), width = 800, height = 300)
makeBarplot(cbind(quarter = lafp[ , 1], lafp[ruleorder[-c(1,2)]] / lafp$count * 1000),
            plotcols[-c(1,2)], legend = TRUE, ymax = 1.2)
dev.off()

# # src, no15, no01, count, mono
# png("./rule_violations_src_no15_no01_count_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = srcp[ , 1], rule = apply(srcp[ , -c(1, which(colnames(lafp)=='Rule 0015'), which(colnames(lafp)=='Rule 0001'), ncol(lafp))], 1, sum)),
#             singlecol, legend = FALSE, ymax = 10000)
# dev.off()
# 
# # src, no15, count, stack
# png("./rule_violations_src_no15_no01_count_stack.png", width = 800, height = 300)
# makeBarplot(srcp[ , c(1, ruleorder[-c(1,2)])],
#             plotcols[-c(1,2)], legend = TRUE, ymax = 10000)
# dev.off()

# Slide 6
# src, no15, per1k, stack
png(paste0(params$directory, "/plots/rule_violations_src_no15_no01_per1k_stack.png"), width = 800, height = 300)
...
# # all, no15, count, mono
# png("./rule_violations_all_no15_no01_count_mono.png", width = 800, height = 300)
# makeBarplot(data.frame(quarter = allp[ , 1], rule = apply(allp[ , -c(1, which(colnames(lafp)=='Rule 0015'), which(colnames(lafp)=='Rule 0001'), ncol(lafp))], 1, sum, na.rm = T)),
#             singlecol, legend = FALSE, ymax = 20000)
# dev.off()
# 
# 
# # all, no15, count, stack
# png("./rule_violations_all_no15_no01_count_stack.png", width = 800, height = 300)
# makeBarplot(allp[ , c(1, ruleorder[-c(1,2)])],
#             plotcols[-c(1,2)], legend = TRUE, ymax = 20000)
# dev.off()
...
write.table(table_all, file = paste0(params$directory, "/plots/rule_violations_table_all"), col.names = F, row.names = F)
# for google slides tables
write_csv(as.data.frame(table_all), paste0(params$directory, "/plots/rule_violations_table_all.csv"))
...
write.table(table_laf, file = paste0(params$directory, "/plots/rule_violations_table_laf"), col.names = F, row.names = F)
write_csv(as.data.frame(table_laf), paste0(params$directory, "/plots/rule_violations_table_laf.csv"))
...
write.table(table_src, file = paste0(params$directory, "/plots/rule_violations_table_src"), col.names = F, row.names = F)
write_csv(as.data.frame(table_src), paste0(params$directory, "/plots/rule_violations_table_src.csv"))

write.table(count_data_side, file = paste0(params$directory, "/plots/unique_dp_counts"), col.names = c("Year", "Unique Extension Elements", "Extension Fact Count", "Unique GAAP Elements", "GAAP Fact Count"))
write_csv(as.data.frame(count_data_side), paste0(params$directory, "/plots/unique_dp_counts.csv"))
...
```
 ...
 <!-- rmarkdown::render("./slides_HTML.Rmd", output_format = "ioslides_presentation", output_file = "test_slides", -->
 <!--                   params = list(directory = "/Users/bradwest/Google_Drive/Projects/DQ_requests/DQC_slides", -->
 <!--                                 username = "web_query", -->
 <!--                                 password = "W0rkiv@", -->
 <!--                                 dbserver = "rltest.markv.com", -->
...
<!-- Make sure that your proper directory is set here so that the plots are sourced correctly -->
